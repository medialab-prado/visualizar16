  ...
  ObjectUtils = {
    ...
    */
    ...
    parseDate: function(str) {
      ...
      if (mnt._pf.unusedTokens.length > 0 || mnt._pf.unusedChars > 2 || mnt._pf.score > 4 || mnt._pf.charsLeftOver > str.length / 2) {
        return null;
      }
      if (mnt.isValid()) {
        return mnt._d;
    ...
    concatenate_recLoop: function(block, result, targetLevel, currentLevel) {
      ...
      if (targetLevel === currentLevel) {
        ...
        for (_i = 0, _len = _ref.length; _i < _len; _i++) {
          qvalue = _ref[_i];
          qvalue.parentLabel = block.label;
        }
      ...
      /*
      blockData.data = result.data
      blockData.metaData = result.metaData
    ...
    concatQvalueArrays: function(a, b) {
      ...
      for (_j = 0, _len1 = b.length; _j < _len1; _j++) {
        ...
        */
        ...
        originalId = qv.id;
        fallbackId = originalId + " (" + qv.parentLabel + ")";
    ...
    */
    ...
    pivot: function(originalBlock, colKey, rowKey, dataKey, aggRule) {
      debugger;
      var c, colCol, colValue, dataCol, dataValue, key1, key2, o1, o2, qv, resultBlockData, rowCol, rowValue, tableObject, _i, _len;
      resultBlockData = {};
      tableObject = {};
      colCol = _.findWhere(originalBlock.data, {
        id: colKey
      }).value.data;
      rowCol = _.findWhere(originalBlock.data, {
        id: rowKey
      }).value.data;
      dataCol = _.findWhere(originalBlock.data, {
        id: dataKey
      }).value.data;
      for (_i = 0, _len = dataCol.length; _i < _len; _i++) {
        qv = dataCol[_i];
        colValue = _.findWhere(colCol, {
          id: qv.id
        });
        rowValue = _.findWhere(rowCol, {
          id: qv.id
        });
        dataValue = _.findWhere(dataCol, {
          id: qv.id
        });
        if (colValue.value === void 0 || colValue.value === "undefined") {
          continue;
        }
        if (tableObject[colValue.value] == null) {
          tableObject[colValue.value] = {};
        }
        if (tableObject[colValue.value][rowValue.value] == null) {
          tableObject[colValue.value][rowValue.value] = this.aggregateInit(dataValue, aggRule, "NADA");
        } else {
          tableObject[colValue.value][rowValue.value] = this.aggregateStep(tableObject[colValue.value][rowValue.value], dataValue.value, aggRule);
        }
      }
      resultBlockData.data = [];
      for (key1 in tableObject) {
        o1 = tableObject[key1];
        c = {
          id: key1,
          value: {
        ...
        resultBlockData.data.push(c);
        for (key2 in o1) {
          o2 = o1[key2];
          o2.value = this.aggregateEnd(o2, aggRule);
          c.value.data.push({
            id: key2,
            value: o2.value.value
          });
        }
      }
      resultBlockData.metaData = originalBlock.metaData;
      resultBlockData.metaData.idLabel = colKey;
      return resultBlockData;
    },
...
(function() {
    ...
    switch (oEvent.data.t) {
      case "operate":
        try {
          ...
          if (oEvent.data.operation === "pivot") {
            resultObj = self.pivotOperation(oEvent);
          }
        } catch (_error) {
          e = _error;
          postMessage({
  ...
  self.concatOperation = function(oEvent) {
    ...
    resultObj.metaData.viewMode = objectsArray[0].metaData.viewMode;
    resultObj.metaData.viewDirection = objectsArray[0].metaData.viewDirection;
    /*
    if true#level == 1
      resultObj.metaData.viewMode = "table"
    else
  ...
  */
  ...
  self.pivotOperation = function(oEvent) {
    debugger;
    var aggRule, colKey, dataKey, originalBlock, resultObj, rowKey;
    originalBlock = JSON.parse(oEvent.data.originalBlock);
    colKey = oEvent.data.colKey;
    rowKey = oEvent.data.rowKey;
    dataKey = oEvent.data.dataKey;
    aggRule = JSON.parse(oEvent.data.aggRule);
    resultObj = ObjectUtils.pivot(originalBlock, colKey, rowKey, dataKey, aggRule);
    return resultObj;
  };
